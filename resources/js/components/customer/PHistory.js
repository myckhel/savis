...
import {payments} from '../../helpers/ajax/customer'
...
export default ({id, route, Table, TableActions}) => {
  ...
  const loadHistory = async () => {
    try {
      const {status, histories} = await payments(id)
  ...
  return (
    loading ? <Loader /> :
    <Table loading={loading} title="Payment History" data={data} config={{
